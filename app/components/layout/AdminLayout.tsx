  ...
  return (
    <div className="admin-layout flex h-screen w-full overflow-hidden bg-slate-50 font-sans">
      ...
      >
        ...
        <div className={`flex items-center gap-3 border-b border-slate-700/50 bg-[#293548] h-20 transition-all px-4 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden py-6 space-y-8 scrollbar-thin scrollbar-thumb-slate-700">
          <div>
            {!isCollapsed && <p className="px-6 text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 transition-opacity duration-300">Quản lý chung</p>}
            <div className="space-y-1">
              <SidebarItem to="/admin/dashboard" icon={<LayoutDashboard size={20} />} label="Tổng quan" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
              <SidebarItem to="/admin/analytics" icon={<BarChart2 size={20} />} label="Thống kê & Báo cáo" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
              <SidebarItem to="/admin/notifications-send" icon={<MessageSquarePlus size={20} />} label="Gửi thông báo" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
            </div>
          </div>

          <div>
            {!isCollapsed && <p className="px-6 text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 transition-opacity duration-300">Đào tạo & Người dùng</p>}
            <div className="space-y-1">
              <SidebarItem to="/admin/classes-management/list" icon={<BookOpen size={20} />} label="Quản lý Lớp học" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
              <SidebarItem to="/admin/subjects-management/list" icon={<Library size={20} />} label="Quản lý Học phần" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
              <SidebarItem to="/admin/users-management/list" icon={<Users size={20} />} label="Quản lý Người dùng" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
            </div>
          </div>

          <div>
            {!isCollapsed && <p className="px-6 text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 transition-opacity duration-300">Hệ thống</p>}
            <div className="space-y-1">
              <SidebarItem to="/admin/audit-logs" icon={<ShieldAlert size={20} />} label="Nhật ký hệ thống" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
              <SidebarItem to="/admin/settings" icon={<Settings size={20} />} label="Cài đặt" isCollapsed={isCollapsed} onClick={() => setIsSidebarOpen(false)} />
            </div>
          </div>
        </div>
      </aside>
